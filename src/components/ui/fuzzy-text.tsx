...
type FuzzyTextDirection = "horizontal" | "vertical" | "both";
...
export function FuzzyText({
  ...
  const canvasRef = useRef<FuzzyTextCanvas | null>(null);
  const text = useMemo(() => Children.toArray(children).join(""), [children]);

  useEffect(() => {
    let animationFrameId: number | undefined;
    let isCancelled = false;
    let glitchTimeoutId: ReturnType<typeof window.setTimeout> | undefined;
    let glitchEndTimeoutId: ReturnType<typeof window.setTimeout> | undefined;
    let clickTimeoutId: ReturnType<typeof window.setTimeout> | undefined;
    ...
    const init = async () => {
      ...
      if (gradient && Array.isArray(gradient) && gradient.length >= 2) {
        ...
        gradient.forEach((gradientColor, index) => {
          grad.addColorStop(index / (gradient.length - 1), gradientColor);
        });
  ...
  return <canvas ref={canvasRef} aria-label={text} className={className} role="img" />;
}
