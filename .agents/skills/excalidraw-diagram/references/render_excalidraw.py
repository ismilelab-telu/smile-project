...
def _setup_command(include_sync: bool) -> str:
    parts = [f"cd {REFERENCES_DIR}"]
    if include_sync:
        parts.append("uv sync")
    parts.append("uv run playwright install chromium")
    return " && ".join(parts)
...
def render(
    ...
    try:
        ...
        print(f"Run: {_setup_command(include_sync=True)}", file=sys.stderr)
        sys.exit(1)
    ...
    with sync_playwright() as p:
        try:
            ...
            if "Executable doesn't exist" in str(e) or "browserType.launch" in str(e):
                ...
                print(f"Run: {_setup_command(include_sync=False)}", file=sys.stderr)
                sys.exit(1)
            raise
