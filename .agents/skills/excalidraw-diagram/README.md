...
Compatible with any coding agent that supports skills. For Codex global installs, place it in `~/.codex/skills/excalidraw-diagram`. For agents that read from `.claude/skills/` (like [Claude Code](https://docs.anthropic.com/en/docs/claude-code) and [OpenCode](https://github.com/nicepkg/OpenCode)), place it in `.claude/skills/excalidraw-diagram`.
...
Clone or download this repo, then copy it into your skill directory:

```bash
git clone https://github.com/coleam00/excalidraw-diagram-skill.git
cp -r excalidraw-diagram-skill ~/.codex/skills/excalidraw-diagram
```

For a project-local `.claude/skills` install instead:
