...
**Setup:** If the user asks you to set up this skill (renderer, dependencies, etc.), see `README.md` for instructions. In this Codex install, the skill lives at `~/.codex/skills/excalidraw-diagram`.
...
**For comprehensive or technical diagrams, you MUST build the JSON one section at a time.** Do NOT attempt to generate the entire file in a single pass. This is a hard constraint — large diagrams can exceed model output limits in a single response, and a comprehensive diagram easily pushes past what is practical to emit in one shot. Even if it fit, generating everything at once leads to worse quality. Section-by-section is better in every way.
...
This outputs a PNG next to the `.excalidraw` file. Then inspect the PNG with the available local image-viewing tool.
